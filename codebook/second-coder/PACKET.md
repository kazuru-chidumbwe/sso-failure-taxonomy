...
Case texts are paraphrased. They are not the Table IV inclusion sentences. Apply the rules; do not keyword-match the table.
...
The additional-observations field is evidence for Table IV **excludes**. Use it. A match on the inclusion phrase alone is not enough.
...
## Table IV — Closed category inclusion and exclusion
...
| edge_callback_consume | Failure in the single-use lifecycle of an RP callback-correlation value or edge callback capability, including non-atomic validation/consumption, TTL expiry or TTL mismatch relative to the modeled callback round trip, or concurrent callback handling | OIDC ID-Token nonce validation without a callback-consumption defect; authorization-code or access-token replay handled by the authorization server/resource server; session-plane confusion without callback replay → session_plane |
...
| Case | Category (Table IV or NEW_LABEL) | Severity (Sev-1 / Sev-2 / Sev-3) | One-line note (optional) |
| ---: | --- | --- | --- |
| 01 |  |  |  |
| 02 |  |  |  |
| 03 |  |  |  |
