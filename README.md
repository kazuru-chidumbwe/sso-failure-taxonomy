...
https://github.com/kazuru-chidumbwe/sso-failure-taxonomy — cite tag **`v1.0.16`** (or its Zenodo version DOI), not floating `main`.
...
`make smoke` runs eight unittests plus the I4 fixture size check. Atomic consume: one concurrent winner, replay rejected. Naive check-then-delete: false-reject window. Presence-only (`presence_only`): second presentation accepted.
