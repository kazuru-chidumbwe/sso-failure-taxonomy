# External incidents against Table IV

## Purpose

This is an **ongoing falsifiability check**, not part of the manuscript’s N=11 catalog.

Operators of constrained multi-site open-source IAM / federated SSO deployments may submit fingerprint-safe incident summaries and ask whether they fit the published ten-category scheme (`codebook/taxonomy.json` / manuscript Table IV) without inventing a new label.

## How to submit

Open a GitHub issue with the **External incident (Table IV)** template, or file a plain issue with the fields below.
...
4. **Proposed Table IV category** — one of the ten ids, or `none` if you believe no cell fits.
5. **Negative evidence** — why neighboring labels do not apply (one sentence each for the closest excludes).
6. **Severity (optional)** — Sev-1 / Sev-2 / Sev-3 using `codebook/taxonomy.json` / Table V meanings.
...
| Outcome | Meaning |
| --- | --- |
| `fits` | Places in exactly one Table IV cell under the published tie-breakers |
